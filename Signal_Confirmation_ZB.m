...
%% Signal Check Pre inj vs post (30 total mins recording. 5 minutes baseline, then injection,)
 %Use this code for signal confirmation in HPCd
%% Import data:
...
B7812_sc.time = B7812_sc_time;
FP.rawdata(:,1) = B7812_sc.time(10:end); 
FP.rawdata(:,2) =  B7812_sc.signal(10:end);
inj = B7812_sc_keydown;
startpoint = FP.rawdata(1,1);
B7812_sc_keydown = (B7812_sc_keydown - startpoint) ./ 1000;
inj = (inj - startpoint) ./ 1000;
...
%% Methods of Neural Processing for GRAB DA IP QUIN.PIROLE ONLY
%Quin.pirole traces with DA GRAB do not match an exponential decay (signal
%rises after injection)
%Thus, Corrcted data is simply calcium dep minus isosbestic

FP.corrected_data(:,1) = FP.calcium_dependent(:,1);
FP.corrected_data(:,2) = FP.calcium_dependent(:,2)-(FP.isosbestic(:,2));
...
%% Methods of Neural Processing for GRAB DA IP Sa.line
%%%%%%%% Methods of neural data processing
%Use this for IP Sa.line 
 % If going to use mutant control sensors, go to MUT Normalizing script
...
%can't happen. Stupid fix is to aD the absolute value of the lowest number
...
%-- not sure why that is, but didn't want to muDy the waters here
%% Methods of Neural Processing GRAB DA MUTANTS ONLY
% GRAB DA MUTant Normalizing (Quin.pirole)
...
%% Referencing behaviors for signal checks
%IF CORRECTED DATA FILE IS UNDER 32000 dimension, go to mut signal checks
...
FP.bl = 6000; %number of datapoints to look at before keydown
FP.seg_dur = 24000; %number of datapoints to look at after keydown
t = [];
temp = [];
FP.sc.B7812_sc_refeeding = []; %necessary if you re-run this part of the code changing baseline or seg_dur
for i = 1
        t  = find(FP.corrected_data(:,1) >= B7812_sc_keydown(i)); %find all FP data indices that are greater than or equal to keystroke timestampe
...
FP.sc.B7812_sc_refeeding(1,:) = FP.corrected_data(temp(ii)-FP.bl:temp(ii)+FP.seg_dur,2)
end
     
% transpose FP.sc.B7812_sc_refeeding 
%% %% Signal Check ANALYSIS
% Set up a time vectorx
FP.time = linspace(-5,20,30001);
%Make the first column your corrected data and the second the time vector:

FP.sc.B7812_sc_refeeding(:,2) = FP.time;

FP.inj = (FP.inj ./ 60);
%FP.inj = 0;

%% Refeeding activity bout quantification analysis: SIGNAL CHECKS
%The following script will be dedicated to quantifying and analyzing the
%activity of a brain region before the injection 4-2 min before keydown and compare
%it to the activity after the inj.

%First, we will want to convert the pre-inj access period processed and finalized trace of a
%session to its own zscore:
%Pre-food access recordings are 5 minutes long, because processed traces
%are 20 data points per second we want points up to 6001:

% DELETE THE TIME COLUMN IN YOUR FP.sc.ANIMALID_REFEEDING FILE OR
% YOU WILL GET AN ERROR.

FP.sc.zscore_pre.B7812_sc_refeeding = zscore(FP.sc.B7812_sc_refeeding(1:6001)); 

%The following will extract the standard deviation and mean of period -4min
%to -2min and we will use that standard deviation and mean of this set to normalize period
%before Drug injection: 

FP.sc.zscore_pre.B7812_sc_refeeding_4to2_std = std(FP.sc.B7812_sc_refeeding(1201:3601));
FP.sc.zscore_pre.B7812_sc_refeeding_4to2_mean = mean(FP.sc.B7812_sc_refeeding(1201:3601));

%Next we will convert the period after the injection to a zscore using
%the std and mean we extracted above: 

FP.sc.zscore_postinj.B7812_sc_refeeding_during = (FP.sc.B7812_sc_refeeding(6002:end) - FP.sc.zscore_pre.B7812_sc_refeeding_4to2_mean) ./ FP.sc.zscore_pre.B7812_sc_refeeding_4to2_std;
...
FP.sc.zscore_finalized.B7812_sc_refeeding = []; %Initialize the variable as an empty matrix

FP.sc.zscore_finalized.B7812_sc_refeeding(1:6001) = FP.sc.zscore_pre.B7812_sc_refeeding;
FP.sc.zscore_finalized.B7812_sc_refeeding(6002:30001) = FP.sc.zscore_postinj.B7812_sc_refeeding_during;

figure

plot(FP.time,FP.sc.zscore_finalized.B7812_sc_refeeding)

FP.sc.zscore_finalized.B7812_sc_refeeding = FP.sc.zscore_finalized.B7812_sc_refeeding;
%TRANSPOSE THIS VARIABLE
%% After transposing zscore finalized 
FP.sc.zscore_finalized.B7812_sc_refeeding(:,2) = FP.time;
  

%% Signal Check AUC analysis
%Timepoints
timepoints_5min = [0:5];
timepoints_4to2min_idx_preinj = [1201;1801;2401;3001;3601;];
timepoints_18to20min_idx_postinj = [21600;22200;22800;23400;24000];

for i = 1:length(timepoints_4to2min_idx_preinj) %Number of seconds being accounted for as shown above.
        
        FP.sc.AUC.B7812_sc_pre(i) = FP.sc.zscore_pre.B7812_sc_refeeding((timepoints_4to2min_idx_preinj(i)),1);
        
end

for i = 1:length(timepoints_18to20min_idx_postinj) %Number of seconds being accounted for as shown above.
        
        FP.sc.AUC.B7812_sc_postinj(i) = FP.sc.zscore_postinj.B7812_sc_refeeding_during((timepoints_18to20min_idx_postinj(i)),1);
        
end

FP.sc.AUC.B7812_sc_pre
FP.sc.AUC.B7812_sc_postinj

%% Signal Check Plot for HPC DA manuscript

figure(1)
plot(FP.refeedsaline.zscore_finalized.B7805_refeeding(:,1));
hold on
plot(FP.sc.zscore_finalized.B7801_refeeding(:,1));
hold on
plot(FP.sc.zscore_finalized.B7810_refeeding(:,1));
hold on
xticklabels({'-10','-5','0','5','10','15','20'})
xline(6000, "LineWidth",2,"LineStyle","--");
ylabel({'dHPC DA binding Normalized z-score'});
xlabel({'Time (minutes)'});
title({'Signal Check'}); %change title for each animal
legend GRAB-Saline GRAB-Quinp Mut-Quinp 
